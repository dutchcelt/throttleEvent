 ...
 *
 * Example: addEventListener('resize', throttleEvent(_handler, 500), false);
 *
 ...
 */
const throttler = (callback, ms, scope = this) => {
	...
	return (...args) => {
		if (!running) running = setTimeout(onTimeOut, Math.max(0, DELAY - FRAME), ...args);
	};
};
